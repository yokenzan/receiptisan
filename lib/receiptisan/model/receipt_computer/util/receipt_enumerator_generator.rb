      ...
      module Util
        class ReceiptEnumeratorGenerator
          class << self
            ...
            # @param shinryou_shikibetsu_codes [Array<String>]
            # @param resource_types [Array<Symbol>]
            # @return [Enumnerator]
            def each_cost_for(receipt:, shinryou_shikibetsu_codes: [], resource_types: [])
              codes = shinryou_shikibetsu_codes.empty? ?
                ReceiptComputer::DigitalizedReceipt::Receipt::ShinryouShikibetsu.all.map(&:code) :
                shinryou_shikibetsu_codes
              Enumerator.new do | y |
                codes.each do | shinryou_shikibetsu_code |
